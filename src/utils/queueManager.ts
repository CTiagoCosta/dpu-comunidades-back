import { App } from "../app";
import { StatusFilaAtendimento } from "../generated/prisma/index";

// Buscar a fila de AGUARDANDO
export async function getQueue() {
  return await App.prisma.filaAtendimento.findMany({
    where: { status: StatusFilaAtendimento.AGUARDANDO },
    orderBy: [
...
export async function addToQueue(userData: {
  ...
  return await App.prisma.filaAtendimento.create({
    data: {
...
export async function startAttendance(queueItemId: string, operadorAtendimentoId: string) {
